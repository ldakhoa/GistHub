...
public final class ParserController {
    ...
    public func gistsFromUrl(withIndex index: Int) -> [Gist] {
        do {
            ...
            for snippet in gistSnippets {
                let gist = try gist(fromSnippet: snippet)
                gists.append(gist)
            }
    ...
    public func gist(fromSnippet snippet: Element) throws -> Gist {
        var gist = Gist()
        var user = User()
    ...
    public func buildPagingUrl(withIndex index: Int) -> URL? {
